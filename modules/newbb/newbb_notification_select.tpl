...
<form name="notification_select" action="<{$xoops_url}>/modules/<{$xoops_dirname}>/<{$xoops_notification.target_page}>" method="post">
    ...
    <input type="hidden" name="not_redirect" value="<{$xoops_notification.redirect_script}>" class="form-control">
    <input type="hidden" name="XOOPS_TOKEN_REQUEST" value="<{php}>echo $GLOBALS['xoopsSecurity']->createToken();<{/php}>" class="form-control">
    ...
    <table class="outer">
        ...
        <tr>
            ...
            <td class="odd">
                ...
                <input type="hidden" name="not_list[<{$index}>][params]" value="<{$category.name}>,<{$category.itemid}>,<{$event.name}>" class="form-control">
        ...
        <tr>
            <td class="foot txtcenter" colspan="3"><input type="submit" name="not_submit" value="<{$lang_updatenow}>" class="form-control"></td>
        </tr>
        </tbody>
    </table>
    <div class="txtcenter">
