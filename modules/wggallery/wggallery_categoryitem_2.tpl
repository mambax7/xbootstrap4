...
<div class='center wgg-album-footer'>
    <a class='btn btn-secondary wgg-btn' href='index.php?op=list&amp;alb_pid=<{$category.id}><{if $subm_id}>&amp;subm_id=<{$subm_id}><{/if}>' title='<{$smarty.const._CO_WGGALLERY_CATS_ALBUMS}>'>
    ...
    <{if $album_showsubmitter}>
        <a class='btn btn-secondary wgg-btn' href='index.php?op=list&amp;subm_id=<{$category.alb_submitter}>' title='<{$smarty.const._CO_WGGALLERY_SUBMITTER}>'>
			...
			<{if $displayButtonText}><{$category.submitter}><{/if}>
        </a>
    <{/if}>
</div>
